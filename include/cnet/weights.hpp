...
namespace cnet::weights {
	...
	public:		
		...
		Var get_derror_dinput(const Var &dE, Shape in) const;
		Mat<float32> get_derror_dinput(const Mat<float32> &dE, Shape in) const;
		Mat<float64> get_derror_dinput(const Mat<float64> &dE, Shape in) const;
		...
		Weights &fit(const Mat<float32> &dE, const Mat<float32> &dW, float64 lr);
		Weights &fit(const Mat<float64> &dE, const Mat<float64> &dW, float64 lr);
		Weights &fit(const Var &dE, const Var &dW, float64 lr);
