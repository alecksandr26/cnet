...
static std::unique_ptr<cnet::afunc::afunc<T>> create_afunc(const std::string &name)
{
	static const std::unordered_map<
		std::string, std::function<std::unique_ptr<cnet::afunc::afunc<T>>()>>
		afunc_map = {
			{ "linear",
...
cnet::layer::dense::dense(std::size_t units)
{
	units_			   = units;
	func_			   = std::make_unique<cnet::afunc::linear<double>>();
	in_			   = 0;
	cnet::layer::layer::built_ = false;
	use_bias_		   = true;
}
...
void cnet::layer::dense<T>::build(std::size_t in_size)
{
	...
	if (use_bias_) B_.resize(units_, 1);

	// By default initi the values between 0.0 to 1.0
	W_.rand(0.0, 1.0);
	if (use_bias_) B_.rand(0.0, 1.0);
...
void cnet::layer::dense<T>::build(std::size_t in_size, T init_val)
{
	...
	if (use_bias_) B_.resize(units_, 1, init_val);
	cnet::layer::layer<T>::built_ = true;
}
...
{
	// Build the layer
	if (!cnet::layer::layer<T>::built_) build(X.get_rows());
	...
	if (!use_bias_) return (*func_)(W_ * X);

	return (*func_)(W_ * X + B_);
}

template<class T>
void cnet::layer::dense<T>::fit_backprop(const cnet::mat<T> &E, double lr,
					 const cnet::mat<T> &A)
{
	if (units_ == 0)
		throw std::invalid_argument("invalid layer: Layer is not initlized");
...
template static std::unique_ptr<cnet::afunc::afunc<double>>
create_afunc(const std::string &name);
