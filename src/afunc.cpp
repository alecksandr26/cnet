...
{
	return _mm256_set_pd(std::exp(vec[0]),
			     std::exp(vec[1]),
			     std::exp(vec[2]),
			     std::exp(vec[3]));
}
...
#pragma omp parallel for
		for (std::size_t i = 0; i < n_ite_4; i += 4) {
			...
			vec = _mm256_div_pd(vec_one, _mm256_add_pd(vec_one,
								   _mm256_exp_pd(_mm256_mul_pd(vec_neg_one,
											       vec))));

			// Store the result back to the matrix
			_mm256_storeu_pd(&y_mat_alloc[i], _mm256_set_pd(vec[0], vec[1], vec[2], vec[3]));
		}
...
#pragma omp parallel for
		for (std::size_t i = 0; i < n_ite_8; i += 8) {
			...
			// Store the result back to the matrix
			_mm256_storeu_pd(&y_mat_alloc[i + 4], _mm256_set_pd(vec[0], vec[1], vec[2], vec[3]));
		}
...
cnet::mat<T> cnet::afunc::sigmoid<T>::derivate(const cnet::mat<T> &X) const
{
	...
	switch (n) {
	case 1:
		y_mat_alloc[0] = (1.0 / (2.0 + exp(- x_mat_alloc[0]) + exp(x_mat_alloc[0])));
		break;
	case 2:
		y_mat_alloc[0] = (1.0 / (2.0 + exp(- x_mat_alloc[0]) + exp(x_mat_alloc[0])));
		y_mat_alloc[1] = (1.0 / (2.0 + exp(- x_mat_alloc[1]) + exp(x_mat_alloc[1])));
		break;
	case 3:
		y_mat_alloc[0] = (1.0 / (2.0 + exp(- x_mat_alloc[0]) + exp(x_mat_alloc[0])));
		y_mat_alloc[1] = (1.0 / (2.0 + exp(- x_mat_alloc[1]) + exp(x_mat_alloc[1])));
		y_mat_alloc[2] = (1.0 / (2.0 + exp(- x_mat_alloc[2]) + exp(x_mat_alloc[2])));
		break;
	case 4: case 5: case 6: case 7:
#pragma omp parallel for
		for (std::size_t i = 0; i < n_ite_4; i += 4) {
			...
			vec4double exp_neg_x = _mm256_exp_pd(_mm256_mul_pd(vec_neg_one, vec));
			vec4double exp_x = _mm256_exp_pd(vec);

			vec4double denominator = _mm256_add_pd(_mm256_add_pd(vec_two, exp_neg_x), exp_x);
		...
		for (std::size_t i = n_ite_4; i < n; i++)
			y_mat_alloc[i] = (1.0 / (2.0 + exp(- x_mat_alloc[i]) + exp(x_mat_alloc[i])));
...
#pragma omp parallel for
		for (std::size_t i = 0; i < n_ite_8; i += 8) {
			...
			denominator = _mm256_add_pd(_mm256_add_pd(vec_two, exp_neg_x), exp_x);
			vec = _mm256_div_pd(vec_one, denominator);
