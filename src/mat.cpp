...
#endif
{
	if (n <= MAT_VEC_SIZE) {
		...
		case 2:
			for (std::size_t i = 0; i < n; i++)
				for (std::size_t j = 0; j < n; j++) {
					a[i * N_B + j / VEC_SIZE][j % VEC_SIZE] = A[i * N + j];
					b[i * N_B + j / VEC_SIZE][j % VEC_SIZE] = B[j * N + i]; // Transpose
				}
			break;
		case 4:
			for (std::size_t i = 0; i < n; i++) {
				for (std::size_t j = 0; j < n; j += VEC_SIZE) {
					cnet::vec4double vecA = _mm256_loadu_pd(&A[i * N + j]);
					_mm256_storeu_pd(&a[i * N_B + j / VEC_SIZE][0], vecA);
					
					cnet::vec4double vecB = _mm256_set_pd(B[(j + 3) * N + i],
									      B[(j + 2) * N + i],
									      B[(j + 1) * N + i],
									      B[(j + 0) * N + i]);
					_mm256_storeu_pd(&b[i * N_B + j / VEC_SIZE][0], vecB);
				}
			}
			break;
		...
		default:
			for (std::size_t i = 0; i < n; i += 2) {
				for (std::size_t j = 0; j < n; j += 2 * VEC_SIZE) {
					...
					cnet::vec4double vecA4 = _mm256_loadu_pd(&A[(i + 1) * N + j + VEC_SIZE]);
					_mm256_storeu_pd(&a[i * N_B + j / VEC_SIZE][0], vecA1);
					_mm256_storeu_pd(&a[i * N_B + (j + VEC_SIZE) / VEC_SIZE][0], vecA2);
					_mm256_storeu_pd(&a[(i + 1) * N_B + j / VEC_SIZE][0], vecA3);
					_mm256_storeu_pd(&a[(i + 1) * N_B + (j + VEC_SIZE) / VEC_SIZE][0],
							 vecA4);
					...
					cnet::vec4double vecB3 = _mm256_set_pd(B[(j + 3) * N + i + 1],
									       B[(j + 2) * N + i + 1],
									       B[(j + 1) * N + i + 1],
									       B[(j + 0) * N + i + 1]);
					
					cnet::vec4double vecB4 = _mm256_set_pd(B[(j + 3 + VEC_SIZE) * N + i + 1],
									       B[(j + 2 + VEC_SIZE) * N + i + 1],
									       B[(j + 1 + VEC_SIZE) * N + i + 1],
									       B[(j + VEC_SIZE) * N + i + 1]);
					_mm256_storeu_pd(&b[i * N_B + j / VEC_SIZE][0], vecB1);
					_mm256_storeu_pd(&b[i * N_B + (j + VEC_SIZE) / VEC_SIZE][0], vecB2);
					_mm256_storeu_pd(&b[(i + 1) * N_B + j / VEC_SIZE][0], vecB3);
					_mm256_storeu_pd(&b[(i + 1) * N_B + (j + VEC_SIZE) / VEC_SIZE][0],
							 vecB4);
				}
			}
			break;
			...
			for (std::size_t i = 0; i < n; i++) {
				for (std::size_t j = 0; j < n; j++) {
					...
					for (std::size_t k = 0; k < N_B; k++)
						s = _mm256_add_pd(s, _mm256_mul_pd(a[i * N_B + k],
										   b[j * N_B + k]));
		...
		default:
			for (std::size_t i = 0; i < n; i += 2) {
				for (std::size_t j = 0; j < n; j += 2) {
					...
					for (std::size_t k = 0; k < N_B; k++) {
						s1 = _mm256_add_pd(s1, _mm256_mul_pd(a[i * N_B + k],
										     b[j * N_B + k]));
						s2 = _mm256_add_pd(s2, _mm256_mul_pd(a[i * N_B + k],
										     b[(j + 1) * N_B + k]));
						s3 = _mm256_add_pd(s3, _mm256_mul_pd(a[(i + 1) * N_B + k],
										     b[j * N_B + k]));
						s4 = _mm256_add_pd(s4, _mm256_mul_pd(a[(i + 1) * N_B + k],
										     b[(j + 1) * N_B + k]));
					}
...
#if __AVX512F__
	alloc_mem_matrix((void **) &vec_mat_alloc, MAT_VEC_SIZE * MAT_VEC_SIZE, sizeof(cnet::vec8double));
	std::memset(vec_mat_alloc, 0, sizeof(cnet::vec8double) * MAT_VEC_SIZE * MAT_VEC_SIZE);
#else
	alloc_mem_matrix((void **) &vec_mat_alloc, MAT_VEC_SIZE * MAT_VEC_SIZE, sizeof(cnet::vec4double));
	std::memset(vec_mat_alloc, 0, sizeof(cnet::vec4double) * MAT_VEC_SIZE * MAT_VEC_SIZE);
#endif
}
...
{
	if (mat_)
		free_mem_matrix((void *) mat_);
	if (vec_mat_alloc)
		free_mem_matrix((void *) vec_mat_alloc);
...
#endif
{
	if (vec_mat_alloc == NULL)
		throw std::out_of_range("uninitialized mat: Mat vec allocation is uninitialized");
...
cnet::mat<T> cnet::mat<T>::operator*(const cnet::mat<T> &B)
{
	...
	// Pad the matrices
	std::size_t max_dimension = std::max(std::max(row_, B.get_rows()), std::max(col_, B.get_cols()));
	std::size_t n = precomputed_log_2_n[fast_log2(max_dimension) + 1];
...
void cnet::mat<T>::operator=(const cnet::mat<T> &B)
{
	...
	// Copy the matrices
	std::memcpy((void *) mat_, (void *) B.get_mat_alloc(), sizeof(T) * B.get_cols() * B.get_rows());
}
...
#if __AVX512F__
template static void strassen_mat_mul(double *p_a, double *p_b, double *p_c, std::size_t n, std::size_t N,
				      cnet::vec8double *a, cnet::vec8double *b);
#else
template static void strassen_mat_mul(double *p_a, double *p_b, double *p_c, std::size_t n, std::size_t N,
				      cnet::vec4double *a, cnet::vec4double *b);
#endif
template static void pad_matrix_to_power_2(const cnet::mat<double> &A, double **p_a, std::size_t n);
