...
std::size_t cnet::model::model::get_num_layers(void)
{
	return num_layers_;
}
...
{
	cnet::model::model<T>::layers_ = new cnet::layer::layer<T> *[MAX_AMOUNT_OF_LAYERS];
}
...
template typename cnet::mat<double> cnet::model::sequential::operator()(const cnet::mat<double> &X);
teamplte
